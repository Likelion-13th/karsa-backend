...
- 또한 사용자가 로그아웃을 하고 다시 로그인을 하여 새로운 토큰으로 교체할 때 updateRefreshToken 함수를
사용하여 새로운 리프레시 토큰을 발급받을 수 있는것같다.
- updateTtl 함수를 통하여 토큰이 만료되는 시간 또한 갱신할 수 있다.

2) 없으면/틀리면?
- 이 파일이 없으면 유저에게 할당된 리프레시 토큰을 저장할 수 없고 토큰을 재발급하거나 시간을 갱신할 수도 없어
로그인 로직이 깨지게 될 것 같다.
 */
